    ...
    return BlocConsumer<InfoRapidaBloc, InfoRapidaState>(
      listener: (context, state) {
        if (state is InfoRapidaError) {
          ...
          showDialog(
            context: context,
            builder: (_) => const DialogLoading(message: "Buscando informacion..."),
          );
          ...
          Navigator.pushReplacementNamed(
            context,
            route,
            arguments: route == 'location-info' ? [state.infoRapidaResult] : null,
          );
        }
      },
              ...
              children: [
                ...
                if (bloc.isKeyboardVisible &&
                  ...
                  CustomKeyboard(
                    ...
                    onchanged: () => bloc.add(SearchProductEvent(bloc.searchControllerProducts.text)),
                  ),
              ],
            ),
          ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildProductRow('Barcode', product.barcode, isError: product.barcode == null || product.barcode!.isEmpty),
                _buildProductRow('Code', product.code, isError: product.code == null || product.code!.isEmpty),
              ],
            ),
          ),
        ),
      ...
      child: BlocBuilder<ConnectionStatusCubit, ConnectionStatus>(
        builder: (context, status) => Column(
          children: [
            ...
            Padding(
              padding: EdgeInsets.only(top: status != ConnectionStatus.online ? 0 : 35),
              child: Row(
                ...
                children: [
                  IconButton(
                    ...
                    onPressed: () {
                      context.read<InfoRapidaBloc>().searchControllerProducts.clear();
                      context.read<InfoRapidaBloc>().add(ShowKeyboardEvent(false));
                      Navigator.pushReplacementNamed(context, 'info-rapida');
                    },
                  ),
                  Padding(
                    padding: EdgeInsets.only(left: size.width * 0.22),
                    child: const Text('PRODUCTOS', style: TextStyle(color: white, fontSize: 18)),
                  ),
                  const Spacer(),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: const [
        ...
        Text('No tiene productos en la base de datos', style: TextStyle(fontSize: 12, color: grey)),
        SizedBox(height: 60),
      ],
    );
  }
